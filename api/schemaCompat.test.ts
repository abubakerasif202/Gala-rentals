...
describe('schemaCompat', () => {
  ...
  it('uses the inspected production schema when the live database is still camelCase', async () => {
    ...
    await expect(getSchemaCompat()).resolves.toMatchObject({
      ...
      applicationStripeCheckoutSessionColumn: 'stripeCheckoutSessionId',
      applicationStripeCustomerColumn: 'stripeCustomerId',
      applicationStripeInvoiceColumn: 'stripeInvoiceId',
      applicationStripePaymentIntentColumn: 'stripePaymentIntentId',
      applicationStripeSubscriptionColumn: 'stripeSubscriptionId',
