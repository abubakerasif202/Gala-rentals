...
export const getCheckoutStatusPresentation = ({
  ...
  if (isError || !hasVerificationContext || !data) {
    return {
      body: 'We could not verify this secure checkout session yet. If Stripe charged the payment, Galarentals can recover it from the checkout session.',
      isFailure: true,
      shouldRefetch: false,
  ...
  if (state === 'complete_paid') {
    return {
      body: 'Your payment has been confirmed. Weekly payments will now be managed through Stripe, and Galarentals will contact you to complete onboarding and handover details.',
      isFailure: false,
      shouldRefetch: false,
      showSecurePaymentLink: false,
      showSpinner: false,
  ...
  if (data.application_status === 'Cancelled') {
    return {
      body: 'This application has been cancelled by Galarentals. If you believe this is a mistake, contact support before trying the link again.',
      isFailure: true,
      shouldRefetch: false,
      showSecurePaymentLink: false,
      showSpinner: false,
  ...
  if (state === 'manual_review') {
    return {
      body:
        'Stripe has already confirmed your payment. We are waiting for Galarentals to complete the final onboarding checks, and this page will keep checking automatically while that finishes. Galarentals will contact you if any manual action is still needed.' +
        (pollingTimedOut ? timeoutSuffix : ''),
      isFailure: false,
      shouldRefetch: !pollingTimedOut,
      showSecurePaymentLink: false,
  ...
  return {
    body: isExpiredOrCanceledCheckout(data)
      ...
      : 'This checkout could not be verified against the secure payment link. Contact Galarentals so the session can be reviewed safely.',
    isFailure: true,
    shouldRefetch: false,
